classdef Sim_AuxiliaryFunctions
    methods
...
%             end
        ...
        function [mag] = circle_setpoints(obj,speed,x_rad,y_rad,r)    
            %CIRCLE
            x_origin = x_rad;
            y_origin = y_rad;
            radius = r;
            ...
            invert_pos(2,:) = flip(circle_final_pose(2,:));
            invert_pos(1,:) = flip(circle_final_pose(1,:));
            
            invert_vel = zeros(2,201);
            invert_vel(2,:) = flip(circle_final_vel(2,:));
            invert_vel(1,:) = flip(circle_final_vel(1,:));
            
            invert_acc = zeros(2,201);
            invert_acc(2,:) = flip(circle_final_acc(2,:));
            invert_acc(1,:) = flip(circle_final_acc(1,:));
            
            invert_jer = zeros(2,201);
            invert_jer(2,:) = flip(circle_final_jerk(2,:));
            invert_jer(1,:) = flip(circle_final_jerk(1,:));
            
            invert_sna = zeros(2,201);
            invert_sna(2,:) = flip(circle_final_snap(2,:));
            invert_sna(1,:) = flip(circle_final_snap(1,:));
