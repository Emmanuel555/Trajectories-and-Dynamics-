...
def minimum_snap_trajectory_generator(traj_derivatives, yaw_derivatives, t_ref, quad, map_limits, plot):
    ...
    len_traj = traj_derivatives.shape[2] # number of waypoints 

    # Add gravity to accelerations
    gravity = 9.81
    thrust = traj_derivatives[2, :, :].T + np.tile(np.array([[0, 0, 1]]), (len_traj, 1)) * gravity # added 9.81 to compensate for gravity in acceleration component for z  
    # Compute body axes
...
def borealis_input_trajectory(current_pose, desired_traj, final_pose, quad, discretization_dt, speed): # might need to add an arg here for n to be inserted 
    #desired_traj = np.concatenate((current_pose,desired_traj))
    #desired_traj = desired_traj
    #print ("desired_traj", desired_traj)
    cu_x = current_pose[0][0]
    cu_y = current_pose[0][1]
    cu_z = current_pose[0][2]

    #de_x = desired_traj[1][0]
    #de_y = desired_traj[1][1]
    #de_z = desired_traj[0][2]
    ...
    #print ("desired:", de_x, de_y, de_z)

    n = 3
    pos_traj = np.zeros((n, 3))
    pos_traj[:, 0] = np.linspace(cu_x, de_x, n)
    pos_traj[:, 1] = np.linspace(cu_y, de_y, n)
    pos_traj[:, 2] = np.linspace(cu_z, de_z, n) # used to be 2.5
    att_traj = np.zeros_like(pos_traj)

    av_dist = np.mean(np.sqrt(np.sum(np.diff(pos_traj, axis=0) ** 2, axis=1)))  # euclidean distance calculated
    av_dt = av_dist / speed # avg time spent/allocated per avg distance travelled between dedicated keyframes from the above where n was defined 

    # TO DO
    poly_pos_traj = fit_multi_segment_polynomial_trajectory(pos_traj.T, att_traj[:, -1].T) # T = transpose
    traj, yaw, t_ref = get_full_traj(poly_pos_traj, target_dt=av_dt, int_dt=discretization_dt)

    #yaw = np.zeros((len(traj), 1)) # same as the one above 

    reference_traj, t_ref, reference_u = minimum_snap_trajectory_generator(traj, yaw, t_ref, quad, None, False)
    return reference_traj, t_ref, reference_u


def straight_trajectory(quad, discretization_dt, speed): # might need to add an arg here for n to be inserted 
    # global path
    n = 5
    pos_traj = np.zeros((n, 3))
    pos_traj[:, 0] = np.linspace(0, 5, n) # (start, end, intermediary waypoints) for target position 
    pos_traj[:, 2] = 3.0
    att_traj = np.zeros_like(pos_traj)
