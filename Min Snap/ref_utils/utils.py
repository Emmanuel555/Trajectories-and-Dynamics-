...
def error_tracking_rmse(error_array,data_size): # solely for position alone 
    ...
    for i in range(data_size):
        ex += np.power(error_listed[i][0],2)
        ey += np.power(error_listed[i][1],2)   
        ez += np.power(error_listed[i][2],2)

    ex = np.sqrt((ex/data_size))
    ey = np.sqrt((ey/data_size))
    ez = np.sqrt((ez/data_size))           
...
def distance_maximizing_points_1d(points, n_train_points, dense_gp=None):
    ...
    for i in range(n_train_points):
        bin_values = points[np.where(hist_indices == i)] # x-raw value
        if len(bin_values) < 1:
...
def distance_maximizing_points_2d(points, n_train_points, dense_gp, plot=False):
    if n_train_points > 30:
        ...
        n_samples = int(np.floor(n_train_points / n_clusters)) # no of samples per cluster?
    else:
...
def make_bx_matrix(x_dims, y_feats):
    ...
    bx = np.zeros((x_dims, len(y_feats))) # size 13 x 1
    for i in range(len(y_feats)): # y_feats = [7 or 8 or 9] of size 1 
        bx[y_feats[i], i] = 1
...
def quaternion_state_mse(x, x_ref, mask):
    ...
    #print("e is ",e)
    #print("e * mask is ",e * np.array(mask))
    #print("e dot e is ",np.dot(e,e))
    #print("combined is ",np.dot(e * np.array(mask),e))
